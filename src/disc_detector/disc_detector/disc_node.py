...
class DiscDetector(Node):
    def __init__(self):
        ...
        self.red_sub = self.create_subscription(
            HsvColor, "/disc_color_red", self.red_color_callback, 10
        )
        self.yellow_sub = self.create_subscription(
            HsvColor, "/disc_color_yellow", self.yellow_color_callback, 10
        )

        self.color_config = {}

        self.red_points_pub = self.create_publisher(
            PolygonStamped, f"/disc_points_red", 10
        )
        self.yellow_points_pub = self.create_publisher(
            PolygonStamped, f"/disc_points_yellow", 10
        )
    ...
    def red_color_callback(self, msg):
        ...
        self.color_config["red"] = {"lower_hsv": msg.lower, "upper_hsv": msg.upper}

    def yellow_color_callback(self, msg):
        self.get_logger().debug("Received yellow color config")
        self.color_config["yellow"] = {"lower_hsv": msg.lower, "upper_hsv": msg.upper}
    ...
    def image_callback(self, msg):
        ...
        disc_data_msg.x = x
        disc_data_msg.y = y
        disc_data_msg.color = ["red"] * len(red_candidates) + ["yellow"] * len(
            yellow_candidates
        )
    ...
    def image_callback_color(self, msg, color):
        ...
        if (
            color in self.color_config
            and "lower_hsv" in self.color_config[color]
            and "upper_hsv" in self.color_config[color]
        ):
            ...
            self.get_logger().warn(
                f"Color config not found for {color}, using defaults"
            )
            if color == "red":
        ...
        if contours:
            ...
            if points_published > 0:
                ...
                self.get_logger().debug(
                    "No circular disc candidates above threshold were published"
                )
